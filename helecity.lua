...
-- ========================
-- СТАТУС ЛЕЙБЛ (слева от кнопки)
-- ========================

local function makeStatusLabel(parent, yBtn)
    local bg = Instance.new("Frame")
    bg.Size = UDim2.new(0, 38, 0, 20)
    bg.Position = UDim2.new(0, -44, 0, yBtn + 14)
...
do local c = Instance.new("UICorner") c.CornerRadius = UDim.new(0,8) c.Parent = gripBar end
...
-- ENABLE: гравитация выкл = зелёный, вкл = красный
local enableBtn, enableStroke = makeInnerBtn("⚡ ENABLE", 0, Color3.fromRGB(80, 255, 80))
enableBtn.Visible = false
local enableStatus = makeStatusLabel(btnContainer, 22 + 0)

-- FLY: выкл = зелёный (готов), вкл = красный (летим)
-- Используем зелёный как "доступно", красный как "активно" для FLY тоже нет —
-- для FLY оставим: выкл=зелёный, вкл=красный (логика та же)
local flyBtn, flyStroke = makeInnerBtn("✈  FLY", 56, Color3.fromRGB(80, 255, 80))
local flyStatus = makeStatusLabel(btnContainer, 22 + 56)
...
zondFrame.ClipsDescendants = false
...
do local c = Instance.new("UICorner") c.CornerRadius = UDim.new(0,8) c.Parent = zondGrip end
...
do local c = Instance.new("UICorner") c.CornerRadius = UDim.new(0,12) c.Parent = zondBtn end
...
local zondStatus = makeStatusLabel(zondFrame, 22)

-- ========================
-- ЦВЕТА КНОПОК + СТАТУС
-- Для всех: выкл = зелёный ("выкл"), вкл = красный ("вкл")
-- ========================

local function setColor(btn, stroke, statusLbl, active)
    local col = active and Color3.fromRGB(255,60,60) or Color3.fromRGB(80,255,80)
    btn.TextColor3   = col
    stroke.Color     = col
    statusLbl.Text   = active and "вкл" or "выкл"
    statusLbl.TextColor3 = active and Color3.fromRGB(255,120,120) or Color3.fromRGB(120,255,120)
end

local function setEnableColor(active)  setColor(enableBtn, enableStroke, enableStatus, active) end
local function setFlyColor(active)     setColor(flyBtn,    flyStroke,    flyStatus,    active) end
local function setZondColor(active)    setColor(zondBtn,   zondStroke,   zondStatus,   active) end

-- Инициальные состояния
...
-- ========================
-- НЕУЯЗВИМОСТЬ (двойной слой: HealthChanged + Heartbeat)
-- ========================
...
local function startNoClip()
    ...
    noClipConn = RunService.RenderStepped:Connect(function()
        ...
        if vehicle then
            for _, part in ipairs(getAllParts(vehicle)) do part.CanCollide = false end
        end
    end)
end
...
local function stopNoClip()
    ...
    local vehicle = getCurrentVehicle()
    if vehicle then
        for _, part in ipairs(getAllParts(vehicle)) do part.CanCollide = true end
    end
end

-- ========================
-- ЗОНД — постоянный NoCollide
-- НЕ вызывать stopZondNoCollide до успешного подбора!
-- ========================
...
local function startZondNoCollide(probe)
    ...
    for _, obj in ipairs(probe:GetDescendants()) do
        if obj:IsA("BasePart") then obj.CanCollide = false obj.CanTouch = false end
    end
...
-- ========================
-- ЗАЩИТА ОТ СДУВАНИЯ (BodyPosition на HRP)
-- ========================

local antiBlowBP = nil

-- Позиция НЕ обновляется автоматически — держим игрока строго на месте.
-- Когда нужно телепортировать — обновляем antiBlowBP.Position вручную перед CFrame.
local function startAntiBlowaway(hrp)
    if antiBlowBP and antiBlowBP.Parent then antiBlowBP:Destroy() end
    local bp = Instance.new("BodyPosition")
    bp.Name     = "_AntiBlowBP"
    bp.Position = hrp.Position   -- фиксируем текущую точку
    bp.MaxForce = Vector3.new(9e5, 9e5, 9e5)
    bp.P        = 6e4
    bp.D        = 1200
    bp.Parent   = hrp
    antiBlowBP  = bp
...
-- Вызывать ДО каждого hrp.CFrame = ... чтобы физика не тянула назад
local function moveAntiBlowaway(pos)
    if antiBlowBP and antiBlowBP.Parent then
        antiBlowBP.Position = pos
    end
end

local function stopAntiBlowaway()
    if antiBlowBP and antiBlowBP.Parent then antiBlowBP:Destroy() end
    antiBlowBP = nil
end
...
-- БАГ ИСПРАВЛЕН: убрали ограничение 30 studs, ищем по Creator/Owner по всему workspace
        ...
        if obj:IsA("Model") then
            for _, name in ipairs(PROBE_TOOL_NAMES) do
                if obj.Name == name or obj.Name == (name .. "Model") then
                    ...
                    -- Запасной вариант: ближайший зонд в мире (без ограничения расстояния)
                    if hrp then
...
local function placeZond()
    ...
    if equipped then
        ...
        if re then pcall(function() re:FireServer(hrp.Position - Vector3.new(0,3,0)) end) end
    ...
    -- Немедленно делаем некол​лайдируемым
    if probe then
        startZondNoCollide(probe)
    end
end

-- Вспомогательная: одна попытка подбора
local function tryPickupOnce(probe, hrp)
    local primary = probe.PrimaryPart or probe:FindFirstChildWhichIsA("BasePart")
    if not primary then return end
    local dest = primary.Position + Vector3.new(0, 3.5, 0)
    moveAntiBlowaway(dest)          -- сначала двигаем цель, потом телепортируем
...
-- БАГ ИСПРАВЛЕН: правильная последовательность:
-- 1. Сохранить позицию машины/сиденья
-- 2. Выйти из машины (hum.Sit = false)
-- 3. Включить защиту от торнадо (BodyPosition)
-- 4. Подобрать зонд с проверкой (до 4 попыток)
-- 5. Вернуться в машину (seat:Sit)
-- 6. Снять защиту
-- 7. Остановить NoCollide зонда (он уже подобран)
local function removeZond()
    ...
    -- Запоминаем машину ДО выхода
    local savedVehicle, savedSeat = getCurrentVehicle()
    local savedSeatCF = savedSeat and savedSeat.CFrame or nil

    -- Выходим из машины
    if savedSeat then
        pcall(function() hum.Sit = false end)
        task.wait(0.2)
        -- Убеждаемся что вышли
        if hum.SeatPart then
            pcall(function()
                hrp.CFrame = savedSeat.CFrame * CFrame.new(0, 4, 0)
            end)
            task.wait(0.15)
        end
    end

    -- Включаем защиту от торнадо (держим персонажа на месте)
    startAntiBlowaway(hrp)

    -- Ищем зонд (NoCollide оставляем АКТИВНЫМ всё время)
    ...
    if probe and probe.Parent then
        ...
        for attempt = 1, MAX_TRIES do
            tryPickupOnce(probe, hrp)
            task.wait(0.4)

            local stillInWorld = probe.Parent ~= nil
            local toolBack     = findProbeTool() ~= nil
            if not stillInWorld or toolBack then
                picked = true
                break
            end

            -- Пробуем с другой позиции
            if attempt < MAX_TRIES then
                local prim = probe.PrimaryPart or probe:FindFirstChildWhichIsA("BasePart")
                if prim then
                    local offsets = {
                        Vector3.new(2,3.5,0),
                        Vector3.new(-2,3.5,0),
                    ...
                    local dest2 = prim.Position + offsets[attempt]
                    moveAntiBlowaway(dest2)
                    hrp.CFrame = CFrame.new(dest2)
                    task.wait(0.15)
                end
            end
        end
    else
        -- Зонд не найден — считаем успешным (возможно уже исчез)
        picked = true
    end

    -- Возвращаемся в машину
    if savedSeat and savedSeat.Parent then
        local returnPos = (savedSeat.CFrame * CFrame.new(0, 1, 0)).Position
        moveAntiBlowaway(returnPos)
        hrp.CFrame = CFrame.new(returnPos)
        task.wait(0.15)
        pcall(function() savedSeat:Sit(hum) end)
        task.wait(0.2)
    end

    -- Снимаем защиту от торнадо
    stopAntiBlowaway()

    -- Теперь можно убрать NoCollide (зонда уже нет)
    stopZondNoCollide()
...
local function toggleFly()
    CarFly = not CarFly
    setFlyColor(CarFly)
    if CarFly then startFly() else stopFly() end
end
...
enableBtn.MouseButton1Click:Connect(function()
    ...
    setEnableColor(HeavyGravity)
    if HeavyGravity then enableHeavyGravity() else disableHeavyGravity() end
end)
...
MainTab:CreateToggle({
    Name = "Car Fly — кнопка FLY появится вверху",
    CurrentValue = false,
    Flag = "CarFlyMaster",
    Callback = function(val)
        ...
        if val then
            ...
            if not (HeavyGravity and enableBtn.Visible) then
                btnContainer.Visible = (enableBtn.Visible)
            end
            stopFly()
        end
    end,
...
MainTab:CreateToggle({
    Name = "Heavy Gravity — кнопка ENABLE появится вверху",
    CurrentValue = false,
    Flag = "HeavyGravToggle",
    Callback = function(val)
        HeavyGravity = val
        setEnableColor(val)
        enableBtn.Visible    = val
        btnContainer.Visible = val or CarFly
        if val then enableHeavyGravity() else disableHeavyGravity() end
    end,
})
...
SettingsTab:CreateToggle({
    ...
    Callback = function(val)
        NoClip = val
        if val and CarFly then startNoClip() elseif not val then stopNoClip() end
    end,
})
...
SettingsTab:CreateLabel("Зонд: выход из авто → защита от торнадо → подбор → возврат")
SettingsTab:CreateLabel("NoCollide зонда активен весь процесс подбора")
SettingsTab:CreateLabel("Неуязвимость: HealthChanged + Heartbeat (двойной слой)")
