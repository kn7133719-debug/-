...
-- Гравитация ON  → красный (опасно, активно)
-- Гравитация OFF → зелёный (готова к включению)
local function setEnableRed()   enableBtn.TextColor3 = Color3.fromRGB(255,60,60)   enableStroke.Color = Color3.fromRGB(255,60,60)   end
...
local function tryPickupProbe(probe, hrp)
...
local function removeZond()
    ...
    if probe and probe.Parent then
        -- До 4 попыток подбора с проверкой после каждой
        local MAX_TRIES = 4
        for attempt = 1, MAX_TRIES do
            tryPickupProbe(probe, hrp)
            task.wait(0.35)

            -- Проверяем: зонд исчез из мира ИЛИ появился в инвентаре?
            local stillInWorld = probe.Parent ~= nil
            local toolBack     = findProbeTool() ~= nil
            if not stillInWorld or toolBack then
                break   -- подобрался, выходим
            end

            if attempt < MAX_TRIES then
                -- Телепортируемся чуть в другое место рядом и пробуем снова
                ...
                if prim then
                    ...
                    hrp.CFrame = CFrame.new(prim.Position + offset)
                    task.wait(0.15)
                end
            end
        end
...
enableBtn.MouseButton1Click:Connect(function()
    ...
    if HeavyGravity then setEnableRed()   enableHeavyGravity()
    else                  setEnableGreen() disableHeavyGravity() end
end)
